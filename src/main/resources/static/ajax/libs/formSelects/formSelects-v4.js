...
(function (layui, window, factory) {
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			...
			var heightStyle = !fs.config.height || fs.config.height == 'auto' ? '' : 'xm-hg style="height: 30px;"';
	...
	Common.prototype.exchangeData = function (id, arr) {
		...
		db[id] = {};
		var result = this.getChildrenList(arr, childrenName, '', [], false);
		return result;
	};
	...
	Common.prototype.renderSelect = function (id, tips, select) {
		...
		if (data[id].config.btns.length) {
			...
			arr.push(['<dd lay-value="" class="' + FORM_SELECT_TIPS + '" style="background-color: #FFF!important;">', this.renderBtns(id, null, '10px'), '</dd>', '<dd lay-value="" class="' + FORM_SELECT_TIPS + ' ' + FORM_DL_INPUT + '" style="background-color: #FFF!important;">', '<i class="xm-iconfont icon-sousuo"></i>', '<input type="text" class="' + FORM_INPUT + ' ' + INPUT + '" placeholder="\u8BF7\u641C\u7D22"/>', '</dd>'].join(''));
		} else {
