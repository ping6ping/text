    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                _pageSize = $.common.isEmpty(options.pageSize) ? 5 : options.pageSize;
            ...
            initZX: function(options) {
                ...
                $('#bootstrap-table').bootstrapTable({
                    ...
                    rightFixedColumns: _rightFixedColumns,              // 是否启用冻结列（右侧）
                    rightFixedNumber: _rightFixedNumber,                // 列冻结的个数（右侧）
                    queryParams: $.table.queryParamsZX,                       // 传递参数（*）
                    columns: options.columns,                           // 显示列信息（*）
                    responseHandler: $.table.responseHandler,           // 在加载服务器发送来的数据之前处理函数
                    onLoadSuccess: $.table.onLoadSuccessZX,               // 当所有数据被加载时触发处理函数
                });
            },
            initSL: function(options,params) {
                $.table._option = options;
                _sortOrder = $.common.isEmpty(options.sortOrder) ? "asc" : options.sortOrder;
                _sortName = $.common.isEmpty(options.sortName) ? "" : options.sortName;
                _pageSize = $.common.isEmpty(options.pageSize) ? 5 : options.pageSize;
                ...
                $('#bootstrap-table').bootstrapTable({
                    ...
                    rightFixedColumns: _rightFixedColumns,              // 是否启用冻结列（右侧）
                    rightFixedNumber: _rightFixedNumber,                // 列冻结的个数（右侧）
                    queryParams: params.queryParams,                       // 传递参数（*）
                    columns: options.columns,                           // 显示列信息（*）
                    responseHandler: $.table.responseHandler,           // 在加载服务器发送来的数据之前处理函数
                    onLoadSuccess: $.table.onLoadSuccessSL, 
                    // 当所有数据被加载时触发处理函数
                });
            },
...
//            },
            ...
            onLoadSuccessSL:function()
            {
            	
            	var contentStart = '<table data-mobile-responsive="true" class="table table-hover"><thead><tr><th data-field="dictId" tabindex="0"><div class="th-inner" style="text-align:center">委托机构</div>'+
						'<div class="fht-cell"></div></th><th data-field="dictName" tabindex="0"><div class="th-inner" style="text-align:center">受理进度</div><div class="fht-cell"></div>'+
					'</th><th data-field="dictType" tabindex="0"><div class="th-inner" style="text-align:center">受理人</div><div class="fht-cell"></div></th><th data-field="dictType" tabindex="0">'+
					'<div class="th-inner" style="text-align:center">受理时间</div><div class="fht-cell"></div></th></tr></thead><tbody>';
            	...
            	var contentEnd = '</tbody></table>';   
            	 ...
            	 $('[data-toggle="popover"]').each(function () {
            		 ...
            		 element.popover({
                         ...
                         placement: 'right', //top, bottom, left or right
                         html: 'true',
                         content: contentStart+showPress(id,flag)+contentEnd,
    	...
    	modal: {
            ...
            // 消息提示并刷新当前窗体
            msgReloadAdd: function(msg, type) {
            	layer.msg(msg, {
            	    icon: $.modal.icon(type),
            	    time: 500,
            	    shade: [0.1, '#8F8F8F']
            ...
            openSL: function (title, url, width, height) {
            	...
            	openFB: function (title, url, width, height) {
            ...
            openGL: function (title, url, width, height) {
            ...
            // 弹出层指定宽度
            openJD: function (title, url, width, height) {
        ...
        operate: {
            ...
            addFB: function(id) {
                ...
                $.modal.openFB("添加" + $.table._option.modalName, url);
            },
            addSL: function(id) {
            	
            	var url = $.common.isEmpty(id) ? $.table._option.createUrlSL : $.table._option.createUrlSL.replace("{id}", id);
                $.modal.openSL("受理进度", url);
            },
            // 修改信息
            edit: function(id) {
            	var url = "/404.html";
            ...
            editGLXQ: function(id) {
            	
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.updateUrlXQ.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
            ...
            editSL: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.updateUrlSL.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
            ...
            successCallbackEdit: function(result) {
            	
                if (result.code == web_status.SUCCESS) {
                    if (window.parent.parent.$("#bootstrap-table").length > 0) {
                        $.modal.closeEdit();
                        window.parent.parent.$.modal.msgSuccess(result.msg);
                        window.parent.parent.$.table.refresh();
                    } else if (window.parent.parent.$("#bootstrap-tree-table").length > 0) {
                        $.modal.close();
                        window.parent.parent.$.modal.msgSuccess(result.msg);
                        window.parent.parent.$.treeTable.refresh();
                    } else {
                        $.modal.msgReload("保存成功,正在刷新数据请稍后……", modal_status.SUCCESS);
                    }
                } else {
